...
until $(docker exec -it re2 sh -c "curl --output /dev/null --silent --head --fail -k https://localhost:9443/v1/bootstrap"); do
    printf '.'
    sleep 3
done
printf ' ready!\n'
...
printf '\nnode 2: '
docker exec -it 2e3 rladmin cluster create name cluster-2.redis-enterprise.com username $ADMIN_U password $ADMIN_P
...
printf '\nnode 3: '
docker exec -it re3 rladmin cluster create name cluster-3.redis-enterprise.com username $ADMIN_U password $ADMIN_P

printf '\nCluster 3 setup complete!\n'
